...
RSpec.describe Cart do
  describe 'Instance Methods' do
    before :each do
      ...
      ItemDiscount.create(item: @ogre, discount: @discount_1)
      ItemDiscount.create(item: @ogre, discount: @discount_2)
      @cart = Cart.new({
        @ogre.id.to_s => 1,
        @giant.id.to_s => 2
        })
    ...
    it '.discounted_subtotal()' do
      ...
      expect(cart.discounted_subtotal(@ogre)).to eq(90)
      expect(cart_1.discounted_subtotal(@ogre)).to eq(160)
      expect(cart_2.discounted_subtotal(@ogre)).to eq(180)
    end
